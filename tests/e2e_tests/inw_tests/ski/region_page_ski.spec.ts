...
for(const SkiRegiondata of SkiRegionData){
  ...
  test.describe(`Ski Region Page (${SkiRegiondata.SourcePath})`, () => {
    ...
    const target = SkiRegiondata.SourcePath.split('\\').pop()
    ?.split('-')
    .map(word => word.charAt(0).toUpperCase() + word.slice(1))
    .join(' ');
    ...
    test(`Header-Footer Test Ski Region Page (${SkiRegiondata.SourcePath})`, {tag: ['@regression'],}, async ({page, ecmsSignInpage, ecmsMainPage, regionPage}) => {
      ...
      await regionPage.Check_Ski_Region_Page_Header();  
      await regionPage.Check_Ski_Region_Page_Footer();
      /////////
      await page.goto(ECMSurl+'/umbraco/login',{ waitUntil: 'domcontentloaded' });
      await ecmsSignInpage.ECMS_Login(process.env.ECMS_USERNAME,process.env.ECMS_PASSWORD);
      await ecmsMainPage.ECMS_Expand_Tree("Ski Holidays", null, country, target, null);
      ...
      await page.goto(SiteURL, { waitUntil: 'domcontentloaded' });
      await regionPage.Check_Ski_Region_Page_Header_Not_Visible();  
      await regionPage.Check_Ski_Region_Page_Footer_Not_Visible();

    });


    test(`Hero Banner Test Ski Region Page (${SkiRegiondata.SourcePath})`, {tag: ['@regression'],}, async ({page, ecmsSignInpage, ecmsMainPage, regionPage}) => {
      test.setTimeout(180000);
    ...
    test(`At a Glance Test Ski Region Page  (${SkiRegiondata.SourcePath})`, {tag: ['@regression'],}, async ({page, pcmsSignInpage, pcmsMainPage, regionPage}) => {
      test.setTimeout(180000);
    ...
    test(`Accordions Test Ski Region Page (${SkiRegiondata.SourcePath})`, {tag: ['@regression'],}, async ({page, ecmsSignInpage, ecmsMainPage, regionPage}) => {
      test.setTimeout(180000);

    ...
    for (const Data of laplandResortData) {
        test(`Broad search proceeds with default values for ${Data.SourcePath} holidays @inw`, async ({ searchResultPage}) => {
            ...
            await test.step(`And: I input search location`, async () => {
                const extractHotelName = (path: string) => {
                    // Split by slashes or backslashes
                    const pathParts = path.split(/[\\\/]/);
                    
                    // Get the last part (hotel name with hyphens)
                    const hotelSlug = pathParts[pathParts.length - 1];
                    
                    // Replace hyphens with spaces and capitalize each word
                    return hotelSlug.split('-').map(word => 
                        word.charAt(0).toUpperCase() + word.slice(1)
                    ).join(' ');
                };
            
                const accommodation = extractHotelName(Data.SourcePath);
                await searchResultPage.searchAnywhere(accommodation);
            });
